...
from tools.phase5_software_smoke import (
    MAX_ITERATIONS,
    PHASE5_PASS_TOKEN,
...
class TestPhase5SoftwareSmoke(unittest.TestCase):
    def test_deterministic_phase5_lifecycle_passes(self):
        output = io.StringIO()
        with contextlib.redirect_stdout(output):
            result = run(SOFTWARE_ONLY_CONFIRMATION, iterations=3)

        self.assertEqual(result["phase"], 5)
        self.assertEqual(result["scope"], "software_only")
        self.assertEqual(result["iterations"], 3)
        self.assertEqual(result["passed"], 3)
        self.assertEqual(result["timer_intents"], 3)
        self.assertEqual(result["manual_overrides"], 3)
        ...
        self.assertIn("PHASE 5 SOFTWARE-ONLY SMOKE PASS: 3/3", output.getvalue())
    ...
    def test_run_never_reads_wall_time_or_sleeps(self):
        ...
        with mock.patch("time.time", side_effect=AssertionError("wall time")):
            with mock.patch("time.monotonic", side_effect=AssertionError("monotonic")):
                with mock.patch("time.sleep", side_effect=AssertionError("sleep")):
                    with contextlib.redirect_stdout(output):
                        result = run(SOFTWARE_ONLY_CONFIRMATION, iterations=1)
        self.assertEqual(result["passed"], 1)

    def test_iterations_are_strictly_bounded(self):
        for value in (None, False, True, 0, -1, MAX_ITERATIONS + 1, "8"):
    ...
    def test_exact_confirmation_is_required_before_core_loading(self):
        ...
        with mock.patch(
            "tools.phase5_software_smoke._load_software_core",
    ...
    def test_heap_collapse_or_unrecovered_drift_cannot_print_pass_token(self):
        ...
        for measurements in variants:
            with self.subTest(measurements=measurements):
                output = io.StringIO()
                with mock.patch(
                    "tools.phase5_software_smoke._memory_free",
        ...
        with mock.patch(
            ...
            with mock.patch(
                "tools.phase5_software_smoke._check_platform_ticks",
                return_value=True,
            ):
                with contextlib.redirect_stdout(output):
    ...
    def test_import_is_inert_and_hardware_independent(self):
        ...
        def guarded_import(name, *args, **kwargs):
            if name in forbidden or name.startswith("protocol."):
                raise AssertionError("forbidden import attempted: {}".format(name))
        ...
        with mock.patch("builtins.__import__", side_effect=guarded_import):
            with contextlib.redirect_stdout(output):
                namespace = runpy.run_path(
                    "tools/phase5_software_smoke.py",
                    run_name="phase5_software_smoke_import_test",
                )
        self.assertIn("run", namespace)
        self.assertEqual(output.getvalue(), "")

    def test_source_imports_only_hardware_free_dependencies(self):
        with open("tools/phase5_software_smoke.py", "r", encoding="utf-8") as handle:
