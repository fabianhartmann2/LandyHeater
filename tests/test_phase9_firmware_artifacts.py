...
PHASE9 = ROOT / "firmware" / "phase9_frozen"
ARTIFACTS = PHASE9 / "artifacts"


class TestPhase9FirmwareArtifacts(unittest.TestCase):
    ...
    def test_full_flash_metadata_is_exact_and_resolvable(self):
        ...
        for relative in pairs.values():
            self.assertTrue((ARTIFACTS / relative).is_file(), relative)
    ...
    def test_combined_image_contains_only_exact_components_and_ff_gaps(self):
        ...
        self.assertEqual(len(application), 2_020_592)
    ...
    def test_phase8_combined_artifact_remains_unchanged(self):
        phase8 = (
            ROOT
            / "firmware/dfr0975u_n16r8/artifacts/firmware.bin"
        ).read_bytes()
        self.assertEqual(
            hashlib.sha256(phase8).hexdigest(),
